...
try {
    ...
    $stmt = $pdo->prepare("UPDATE reservations SET status = 'refused' WHERE id = ?");
    $stmt->execute([$reservation_id]);
    ...
    header("Location: dashboard.php?success=Réservation refusée et email envoyé&sort=$sort&order=$order" . ($search ? "&search=" . urlencode($search) : ""));
    exit;
} catch (Exception $e) {
