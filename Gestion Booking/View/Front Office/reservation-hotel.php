<?php
// Reservation-hotel.php - Form for booking a hotel

// Get hotel type from URL parameters
$hotelType = isset($_GET['type']) ? $_GET['type'] : '';
$categorie = isset($_GET['categorie']) ? $_GET['categorie'] : 'hotel';

// Hotel details based on type
$hotelDetails = [
    'palace' => [
        'type' => 'Hôtel Palace 5*',
        'chambres' => 'Suites Deluxe/Executive',
        'restaurants' => '3',
        'services' => 'Spa complet, Piscine intérieure & extérieure, Service 24/7',
        'prix_nuit' => '800',
        'petit_dejeuner' => 'Buffet inclus'
    ],
    'boutique' => [
        'type' => 'Hôtel Boutique 4*',
        'chambres' => 'Standards/Supérieures',
        'restaurants' => '1',
        'services' => 'Bar lounge, Terrasse panoramique, Service conciergerie',
        'prix_nuit' => '450',
        'petit_dejeuner' => 'Buffet inclus'
    ],
    'resort' => [
        'type' => 'Résort 5*',
        'chambres' => 'Bungalows/Villas',
        'restaurants' => '4',
        'services' => 'Piscines, Plage privée, Activités nautiques, Spa complet',
        'prix_nuit' => '650',
        'petit_dejeuner' => 'Option all-inclusive disponible'
    ]
];

// Default to Palace if type not specified or invalid
if (!array_key_exists($hotelType, $hotelDetails)) {
    $hotelType = 'palace';
}

$details = $hotelDetails[$hotelType];
...
<head>
    ...
    <title>TuniFy Village - Réservation d'Hôtel</title>
        ...
        <div class="container contact-container">
            <?php if ($formSubmitted): ?>
                ...
                <div style="text-align: center; max-width: 800px; margin: 0 auto; padding: 50px 20px;">
                    ...
                    <p class="about-text" style="margin-bottom: 30px;">
                        Merci pour votre réservation à <?php echo htmlspecialchars($details['type']); ?>. 
            ...
            <?php else: ?>
                <div class="contact-info">
                    ...
                    <h2 class="section-title">Réserver votre <span>Séjour Hôtelier</span></h2>
                    ...
                    <div class="villa-summary" style="margin-top: 30px; background: rgba(201, 168, 108, 0.1); padding: 25px; border-radius: 5px; border: 1px solid rgba(201, 168, 108, 0.3);">
                        <h3 style="color: var(--gold-primary); margin-bottom: 15px; font-size: 20px;">Détails de l'Hôtel</h3>
                        <table style="width: 100%; border-collapse: collapse;">
                            ...
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><strong>Chambres</strong></td>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><?php echo htmlspecialchars($details['chambres']); ?></td>
                            </tr>
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><strong>Restaurants</strong></td>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><?php echo htmlspecialchars($details['restaurants']); ?></td>
                            </tr>
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><strong>Petit-déjeuner</strong></td>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><?php echo htmlspecialchars($details['petit_dejeuner']); ?></td>
                            </tr>
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><strong>Services</strong></td>
                                <td style="padding: 10px 0; border-bottom: 1px solid rgba(201, 168, 108, 0.2);"><?php echo htmlspecialchars($details['services']); ?></td>
                            </tr>
                            <tr>
                                <td style="padding: 10px 0;"><strong>Prix par nuit</strong></td>
                                <td style="padding: 10px 0; color: var(--gold-primary); font-weight: bold;">À partir de <?php echo htmlspecialchars($details['prix_nuit']); ?> DT</td>
                            </tr>
                        </table>
                    </div>
                ...
                <div class="contact-form">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?type=' . htmlspecialchars($hotelType); ?>" method="post">
                        ...
                        <!-- Hidden fields to store hotel information -->
                        <input type="hidden" name="hotel_type" value="<?php echo htmlspecialchars($details['type']); ?>">
                        <input type="hidden" name="type_logement" value="hotel">
                        ...
                        <div class="form-group">
                            <select name="type_chambre" class="form-control" required>
                                ...
                                <?php elseif ($hotelType == 'boutique'): ?>
                                    <option value="standard">Chambre Standard</option>
                                    <option value="superieure">Chambre Supérieure</option>
                                    <option value="terrasse">Chambre avec Terrasse</option>
                                <?php elseif ($hotelType == 'resort'): ?>
                                    <option value="bungalow">Bungalow</option>
                                    <option value="villa_suite">Villa Suite</option>
                                    <option value="plage">Chambre Vue Plage</option>
                                    <option value="famille">Suite Familiale</option>
                                <?php endif; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <select name="formule" class="form-control">
                                <option value="" disabled selected>Formule de séjour</option>
